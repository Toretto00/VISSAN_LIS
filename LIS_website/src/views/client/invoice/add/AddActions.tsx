...
import { postInvoice } from '@/stores/invoice'
...
const AddActions = () => {
    ...
    const handleSendRequest = async (e: SyntheticEvent) => {
        ...
        selectDataInvoice.forEach(element => {
            invoice.push({
                product: {
        ...
        try {
            ...
            const res = await postInvoice(JSON.stringify(invoice), session?.user.name, session?.user.store.storeid)

            if (!res.ok) {

                throw new Error("Fail to post new invoice!")
            }

            setSelectDataInvoice([])

            router.push("/client/invoice/list")
                ...
                <Card>
                    <CardContent className='flex flex-col gap-4'>
                        <Button
                            ...
                            disabled={selectDataInvoice.length === 0}
                        >
                            Send Invoice
                        </Button>
                        <Button
