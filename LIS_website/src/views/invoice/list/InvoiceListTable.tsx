...
import { getInvoiceClient, deleteInvoice } from '@/stores/invoice'
...
const InvoiceListTable = ({ tableData, storeLocations }: { tableData: InvoiceType[], storeLocations: StoreLocationType[] }) => {
      ...
      <Card>
        ...
        <div className='flex justify-between flex-col items-start md:flex-row md:items-center p-6 border-bs gap-4'>
          ...
          <Button
            ...
            // disabled={!startDate && status === "" && endDate.format("DD/MM/YYYY") === dayjs().format("DD/MM/YYYY")}
            onClick={async (e) => {
              e.preventDefault()
              ...
              const data = await getData("", "", dayjs().format("DD/MM/YYYY").toString(), status)
          ...
          <Button
            ...
            // disabled={!startDate && status === "pending" && endDate.format("DD/MM/YYYY") === dayjs().format("DD/MM/YYYY")}
            onClick={async (e) => {
              e.preventDefault()

              const data = await getData("", (!startDate ? "" : startDate.format("DD/MM/YYYY").toString()), endDate.format("DD/MM/YYYY").toString(), status === "all" ? "" : status)
